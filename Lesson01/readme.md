...
1. 主循环五步顺序不能颠倒，尤其清屏必须在绘制之前，否则产生残影。
2. 双缓冲：`pygame.display.flip()` 交换前后台缓冲区，避免屏幕闪烁。
3. 帧率控制：`pygame.time.Clock().tick(xx)` 使循环每秒最多执行 xx 次，稳定游戏速度，降低CPU占用。
